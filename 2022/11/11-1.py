...
class Monkey:
  ...
  def _evaluate(self,math:str) -> Union[float,int]:
    ...
    match groups[1]:
      case "+":
        print(f"    Worry level increases by {y} to {x+y}.")
        return x+y
      case "*":
        print(f"    Worry level is multiplied by {y} to {x*y}.")
        return x*y
  ...
  def processTest(self,item:int) -> bool:
    ...
    if res%div==0:
      print(f"    Current worry level is divisible by {div}.")
    else:
      print(f"    Current worry level is divisible by {div}.")
    return res%div==0,res
  ...
  def processInventory(self) -> None:
    ...
    for item in self.inv.copy():
      ...
      if ok:
        print(f"    Item with worry level {worry} is thrown to monkey {self.test_t}.")
        monkies[self.test_t].inv.append(worry)
        self.inv.remove(item)
        self.inspected += 1
      else:
        print(f"    Item with worry level {worry} is thrown to monkey {self.test_f}.")
